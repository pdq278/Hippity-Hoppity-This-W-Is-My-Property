...
def grabScreen():
    ...
    img = None
    with mss.mss() as sct:
        monitor = {"top": 300, "left": 885, "width": 395, "height": 480}
...
def frameToState(action, currentTime):
    ...
    img = np.array(grabScreen()).reshape(300, 300, 1) # 640 x 480
    img = imgToTensor(img)
    ...
    if (terminal):
        ...
        reward = \
            (1+getTimeReward(1+currentTime))*(1+model.timeSinceLastJump)**2
...
class NeuralNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        self.replay_memory_size = 5000 # Number of experiences we will store in the replay memory.
        self.minibatch_size = 32 # Number of experiences we will sample from the replay memory.
        ...
        self.conv3 = nn.Conv2d(16, 14, 8,  8)
        self.relu3 = nn.ReLU(inplace=True)
        self.conv4 = nn.Conv2d(14, 64, 10,  16)
        self.relu4 = nn.ReLU(inplace=True)
        self.fc4 = nn.Linear(64, 1024)
